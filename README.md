...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    body { background:#000; overflow-x:hidden; color:#fff; font-family:'Inter',sans-serif; }
    ...
    h1 { font-size:80px; font-weight:900; letter-spacing:8px; animation:glitch 5s infinite; background:linear-gradient(135deg,#fff,#aaa); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }
    ...
    .section-title { font-size:50px; font-weight:900; margin-bottom:50px; text-align:center; background:linear-gradient(135deg,#fff,#aaa); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }
  ...
  <script>
    (function() {
      ...
      document.querySelectorAll('.product-card').forEach(function(card) {
        ...
        daysBtns.forEach(function(btn) {
          btn.addEventListener('click', function() {
            daysBtns.forEach(function(b) { b.classList.remove('active'); });
      ...
      downloadLauncher.addEventListener('click', function() {
        alert('Скачивание лаунчера начнется... (демо)');
      });

      buyClientBtn.addEventListener('click', function() {
        hideCabinet();
        document.getElementById('plans').scrollIntoView({ behavior: 'smooth' });
      });

      navPlans.addEventListener('click', function(e) {
        e.preventDefault();
      ...
      console.log('✅ Сайт полностью готов для GitHub Pages.');
    })();
  </script>
</body>
</html>
